...
class AuthoringDerivationDSLParseError(Exception):
    def __init__(
        self,
        message: str,
        *,
...
def parse_authoring_derivation_dsl_v1(source: str) -> dict[str, Any]:
    if not isinstance(source, str):
        raise AuthoringDerivationDSLParseError("source must be string", path="$", kind="input_type")
    try:
        module = ast.parse(source, mode="exec")
    except SyntaxError as exc:
        path = "$" if exc.lineno is None else f"$.dsl:line:{exc.lineno}"
        raise AuthoringDerivationDSLParseError(str(exc), path=path, kind="syntax") from exc
    ...
    for idx, node in enumerate(body_nodes):
        ...
        if isinstance(node, ast.Expr):
            if isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
                continue
            payload = _parse_derivation_expr(node.value, path=f"$.dsl.body[{idx}]")
            if found is not None:
                raise _err("multiple Derivation(...) declarations found", path=f"$.dsl.body[{idx}]")
            found = payload
            continue
        if isinstance(node, ast.Assign):
            ...
            payload = _parse_derivation_expr(node.value, path=f"$.dsl.body[{idx}]")
            if "derivation_id" not in payload and "name" not in payload:
    ...
    if found is None:
        raise _err("no Derivation(...) declaration found", path="$.dsl.body")
    return found


def _parse_derivation_expr(node: ast.AST, *, path: str) -> dict[str, Any]:
    if not isinstance(node, ast.Call):
        raise _err("expected Derivation(...) call", path=path)
    if not (isinstance(node.func, ast.Name) and node.func.id == "Derivation"):
        raise _err("expected Derivation(...) call", path=path)
    if node.args:
        raise _err("Derivation(...) positional arguments are not supported", path=f"{path}.args")
    out: dict[str, Any] = {}
    for kw_idx, kw in enumerate(node.keywords):
        if kw.arg is None:
            raise _err("Derivation(...) **kwargs are not supported", path=f"{path}.keywords[{kw_idx}]")
        if kw.arg in out:
...
def _literal(node: ast.AST, *, path: str) -> Any:
    ...
    if isinstance(node, ast.Call):
        if path.endswith(".head"):
            return _head_call(node, path=path)
        return _dsl_call(node, path=path)
    raise _err("unsupported expression in Derivation(...) DSL", path=path)
...
def _where_literal(node: ast.AST, *, path: str, record_var_types: dict[str, str]) -> Any:
    if isinstance(node, ast.List):
        ...
        for elt in node.elts:
            out.append(_where_atom(elt, path=f"{path}[]", record_var_types=local_types))
        return out
    if isinstance(node, ast.Tuple):
...
def _where_dsl_call(node: ast.Call, *, path: str, record_var_types: dict[str, str]) -> Any:
    ...
    simple_binops = {"Eq": "eq", "Gt": "gt", "Ge": "ge", "Lt": "lt", "Le": "le"}
...
def _head_call(node: ast.Call, *, path: str) -> dict[str, Any]:
    if node.args:
        raise _err("Derivation head call positional arguments are not supported", path=f"{path}.args")
    callee = _head_callee(node.func, path=path)
    kwargs: dict[str, Any] = {}
    for kw_idx, kw in enumerate(node.keywords):
        if kw.arg is None:
            raise _err("Derivation head call **kwargs are not supported", path=f"{path}.keywords[{kw_idx}]")
        if kw.arg in kwargs:
            raise _err(f"duplicate head keyword: {kw.arg}", path=f"{path}.{kw.arg}")
        kwargs[kw.arg] = _head_term(kw.value, path=f"{path}.{kw.arg}")
    if not kwargs:
        raise _err("Derivation head call must include keyword arguments", path=path)
    return {
        "kind": "head_call",
        **callee,
        "kwargs": kwargs,
...
def _head_callee(node: ast.AST, *, path: str) -> dict[str, Any]:
    if isinstance(node, ast.Name):
        return {
    ...
    if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Name):
        return {
            ...
            "entity_type": node.value.id,
            "field": node.attr,
        }
    raise _err("Derivation head must be EntityType(...) or EntityType.field(...)", path=path)


def _head_term(node: ast.AST, *, path: str) -> Any:
    if isinstance(node, ast.Name):
        return f"${node.id}"
    if isinstance(node, ast.Constant):
        if isinstance(node.value, (str, int, bool, float, type(None))):
            return node.value
        raise _err("unsupported head literal type", path=path)
    if isinstance(node, ast.List):
        return [_head_term(x, path=f"{path}[]") for x in node.elts]
    if isinstance(node, ast.Tuple):
        return tuple(_head_term(x, path=f"{path}[]") for x in node.elts)
    raise _err("unsupported head term expression", path=path)
...
def _helper_err(
    ...
    *,
    ...
    return AuthoringDerivationDSLParseError(message, path=path, kind="structure", details=details)


def _err(message: str, *, path: str) -> AuthoringDerivationDSLParseError:
    return AuthoringDerivationDSLParseError(message, path=path, kind="structure")
