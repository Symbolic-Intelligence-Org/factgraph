...
from .assertions import AuditAssertionIndex
...
def extend_schema_ir_with_ecss_vcd_predicates(schema_ir: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(schema_ir, dict):
        raise AuditComplianceError("schema_ir must be dict")

    predicates = schema_ir.get("predicates")
    projection = schema_ir.get("projection")
    if not isinstance(predicates, list):
        raise AuditComplianceError("schema_ir.predicates must be list")
    if not isinstance(projection, dict):
        raise AuditComplianceError("schema_ir.projection must be object")
    projection_predicates = projection.get("predicates")
    if not isinstance(projection_predicates, list):
        raise AuditComplianceError("schema_ir.projection.predicates must be list")
