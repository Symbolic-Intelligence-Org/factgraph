...
面向对接 factpy-kernel HTTP service 的前端/BFF 开发者。本文档把分散在 `01_overview.md` / `02_runtime_sessions.md` / `03_runtime_queries_views.md` / `04_rules_registry.md` / `05_extraction.md` 里的信息,整合成"怎么真正调"。

**机读契约**:[`docs/api/openapi.yaml`](../../../../docs/api/openapi.yaml)(OpenAPI 3.0,48 个 operation 全覆盖)。可直接喂 Swagger UI / Stoplight / `openapi-typescript` 等工具。本文档是"怎么读这份 spec + 典型用法",不是 spec 本身。
...
interface ErrorEntry {
  kind: string;                // 错误类别:shape / validation / runtime / extraction 等
  path: string;                // JSON-pointer-like 路径,或 "$" 表示全局
  details?: Record<string, unknown>;
}
```

**唯一例外**:`POST /v1/extraction/documents` 用 HTTP 状态码区分失败类型:

| 状态码 | 场景 |
|---|---|
| 200 | 抽取成功 |
| 422 | 请求形状问题(`options` 不是合法 JSON / 缺 `schema_ir`) |
| 500 | 抽取管道失败(`stage=staging` / `stage=extraction` / `stage=resolution`) |

422/500 body 仍然是 `{ok: false, errors: [...], meta: {}}`,只是状态码不是 200。
...
| 422 | **仅** extraction 的请求形状问题 | Envelope with ok=false |
| 500 | **仅** extraction 的管道失败 | Envelope with ok=false |
| 503 | `FACTPY_KERNEL_API_KEYS` 未配置 | `{detail: string}` |

## 1. 典型调用链路

### 1.1 Chain A:抽一份文档(最简单路径,单次请求)

```ts
const formData = new FormData();
formData.append("file", fileBlob, "doc.pdf");
formData.append("options", JSON.stringify({
  schema_ir: compiledSchemaIr,                    // 必填:pre-compiled schema IR
  model: "mistral/mistral-small-latest",          // 可选
  entity_descriptions: { Regulation: "A specific article" },
  enable_gleaning: true,
  enable_alias_merge: true,
}));

const resp = await fetch(`${BASE}/v1/extraction/documents`, {
  method: "POST",
  headers: { "X-FactPy-API-Key": apiKey },
  body: formData,
});

// 注意:不能用 resp.ok — 那在 200/422/500 上语义和 envelope.ok 不一致
const body = await resp.json();
if (resp.status === 200 && body.ok) {
  const { facts, entities, metrics } = body.result;
  // 渲染
} else {
  handleError(resp.status, body.errors);    // 见 §2
}
```

完整 DTO:[`05_extraction.md`](./05_extraction.md)。

### 1.2 Chain B:打开 session → 写入 facts → 查询
...
完整 DTO:[`02_runtime_sessions.md`](./02_runtime_sessions.md) §5.2 / [`03_runtime_queries_views.md`](./03_runtime_queries_views.md)。
...
| `validation` | extraction 422:`options` / `options.schema_ir` 问题 |
...
| `extraction` | extraction 500:`details.stage` 指示 staging/extraction/resolution |
...
### 3.2 Extraction `options` 是 JSON 字符串,不是 form field object

很多 form 库会把 `{schema_ir: {...}, model: "..."}` 展平成多个 form fields。**正确做法是把整个 options object 先 `JSON.stringify`,再作为单个 string form field 放进去**。后端 route 层会 `json.loads(options)`。
...
尤其 extraction:`resp.ok` 在 422/500 上会是 `false`,但 422/500 的 body 仍然是 envelope(不同于 401/503)。

### 3.4 `derivations/accept` 要完整 echo candidate
...
- [`03_runtime_queries_views.md`](./03_runtime_queries_views.md) — runtime queries / views DTO 细节
- [`04_rules_registry.md`](./04_rules_registry.md) — rules / registry DTO 细节
- [`05_extraction.md`](./05_extraction.md) — `POST /v1/extraction/documents` DTO 细节
- [`../../../../docs/api/openapi.yaml`](../../../../docs/api/openapi.yaml) — **机读契约(本文档的权威对应物)**
