...
- 最后更新：2026-02-23
- 评估基线：目录重构完成、`Store` 拆分阶段 1 完成、`Ledger` 索引优化完成
- 验证状态：`python -m unittest discover -s src/factpy_kernel/tests -p 'test_*.py'` -> `338 tests OK`
...
| 正确性与防御式校验 | 9.0 | `SchemaIR` 校验、写协议校验、where 校验较严格 |
| 可测试性与回归保障 | 9.0 | 全量 `unittest` 338 通过；新增 core 边界与索引测试 |
| 可扩展性 | 7.5 | 协议与模块分层良好，但 where Python/adapter 双实现仍需长期同步 |
| 性能（当前实现） | 6.8 | `Ledger` 已有索引优化，但仍为内存实现，部分导出路径仍有全量扫描 |
| **综合（主观加权）** | **8.3** | 语义成熟，工程形态已具备持续迭代基础 |
