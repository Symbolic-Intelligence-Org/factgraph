...
- Assessment baseline: directory refactor complete, `Store` phase-1 decomposition complete, `Ledger` indexing optimization complete
- Validation status: `python -m unittest discover -s src/factpy_kernel/tests -p 'test_*.py'` -> `338 tests OK`
...
- Full `unittest` suite passes: `338 tests OK`
