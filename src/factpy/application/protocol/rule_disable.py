...
_RULE_DISABLE_STATUSES = ("completed", "unsupported", "invalid_request")
...
class RuleDisableResult:
    ...
    def __post_init__(self) -> None:
        status = _require_literal(
            self.status,
            field_name="status",
            allowed=_RULE_DISABLE_STATUSES,
        )
        object.__setattr__(
            self,
            "variant_rows",
        ...
        if status == "completed":
            if self.proof_frame is None:
                raise ProtocolShapeError("completed RuleDisableResult requires proof_frame")
            if self.errors:
                raise ProtocolShapeError("completed RuleDisableResult must not contain errors")
            return

        if self.variant_rows:
            raise ProtocolShapeError(f"{status} RuleDisableResult requires empty variant_rows")
        if self.proof_frame is not None:
            raise ProtocolShapeError(f"{status} RuleDisableResult requires proof_frame=None")
        if not self.errors:
            raise ProtocolShapeError(f"{status} RuleDisableResult requires errors")
