# Authoring module overview (factpy)

- Scope: `src/factpy/authoring`
...
- `factpy.application.authoring_runtime` owns the application-layer
  save/load/list/get orchestration used by the SDK facade.
- `factpy.application.workspace_runtime` owns workspace layout, manifest
...
- `factpy.sdk.registry.SDKRegistry` remains an advanced/internal wrapper for
...
- `sdk`
  ...
  - advanced code may still import `factpy.sdk.registry.SDKRegistry`
- `service`
  - The service layer can expose the authoring registry as a
    front-end-consumable interface
- `ecss`
  - `factpy.ecss` provides shared domain presets (e.g. ECSS VCD
