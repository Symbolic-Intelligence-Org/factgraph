...
`factpy.sdk` is the ergonomic product surface, but the factpy exposes
...
- **LLM-generated payloads** — when the calling code is producing
  ...
  read `factpy.application.protocol` before composing.
...
Each of these is reachable via direct import from the relevant factpy
...
read `factpy.audit.reader` for the full field list.
...
`factpy/application/protocol/derivation_fact_overlay.py` before
...
`factpy.application.protocol.CompiledDerivationPlan` directly and feed it
...
```python
import factpy.adapters.souffle    # registers "souffle" mode
import factpy.adapters.problog    # registers "problog" mode
import factpy.adapters.pyreason   # registers "pyreason" mode
```
...
and the factpy team prefers to add them after seeing real-world usage
patterns.
