# Audit Module Overview (factpy)

- Scope: `src/factpy/audit`
- Last updated: 2026-05-06
- Audience: developers consuming an audit package, running offline audit queries, or constructing audit DTOs

## 1. Module Responsibilities

> **Boundary — v0.1 factpy-only wheel**
>
> This document describes how `factpy.audit` integrates with downstream consumers. The following modules are referenced below but are **not part of the v0.1 factpy-only wheel**:
>
...
>
> Calling these modules directly from a factpy-only install raises `ModuleNotFoundError`. `AuditQuery.list_compliance_matrix(...)` raises `AuditOptionalDomainError` to give an actionable signal instead of letting users hit a bare import failure.
...
Batch 8 public-surface note: `factpy.audit` is an **advanced importable** audit consumer surface in the factpy package. Round event recording, round-event queries, and ProofFrame diff are documented audit/query APIs, but they are not mirrored as SDK facade methods in v0.1. Service / static-site delivery remains outside the factpy-only public package.

Related contract documents:

- `src/factpy/audit/docs/03_audit_package_contract.md`
...
The recorder uses the caller-supplied `round_id` plus a per-round monotonic `sequence`. Capability runtimes do not import `factpy.audit`; the caller explicitly records events after each capability returns.
...
In the factpy-only v0.1 wheel, `domains.ecss` is not part of the install. This entrypoint is preserved as a monorepo / optional-domain compatibility surface; if `domains.ecss` is missing, the call raises `AuditOptionalDomainError` rather than treating `domains` as a factpy-required dependency:
...
Full static-site rendering does not belong to the `factpy.audit` module. `service.static_ui` consumes `factpy.audit` reader / query / DTO and domain-backed compliance rows, and owns rendered-site contracts such as `site_manifest.json` / `ui_index.json`.
...
- `ecss`
  ...
  - audit lazily imports and exposes `AuditQuery.list_compliance_matrix(...)` but does not own ECSS row semantics; on a factpy-only wheel where `domains.ecss` is missing, this entrypoint raises `AuditOptionalDomainError`
- `explainability`
