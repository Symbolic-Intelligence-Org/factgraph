...
- Owner: `factpy.audit` reader / query / DTO surface

`factpy.audit` consumes exported audit packages. It does not export packages, render the full static site, query live runtime state, or own domain-specific ECSS row semantics.
...
Batch 8 public-surface note:these query-derived surfaces are `factpy.audit` advanced importable APIs. They are part of the factpy audit reader/query layer,not SDK product facade methods and not service routes.
...
In the factpy-only v0.1 wheel, `domains.ecss` is not part of the installed package set. Calling this convenience without the optional domain package raises `AuditOptionalDomainError` with an actionable message. factpy consumers should treat this surface as domain-backed and optional, not as a guaranteed factpy-only contract.
...
- `factpy.audit`: package read/query/DTO/evidence graph consumer contracts
