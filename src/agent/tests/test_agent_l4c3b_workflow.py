"""Layer 4C3-b batch extraction workflow tests."""
...
class AgentLayer4C3bWorkflowTests(unittest.TestCase):
    def setUp(self) -> None:
        reset_runtime_sessions_for_tests()
        self.tmp = TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.db_path = Path(self.tmp.name) / "agent_l4c3b.sqlite3"
    ...
    def test_extract_from_segments_is_pure_and_does_not_checkpoint(self) -> None:
        staging = self._stage()
        before = self.agent_session.last_active_at
        result = self.orchestrator.extract_from_segments(segments=list(staging.segments))
        self.assertEqual(result.total_segments, len(staging.segments))
    ...
    def test_extract_and_create_document_bundle_then_commit(self) -> None:
        staging = self._stage()
        result, bundle = self.orchestrator.extract_and_create_document_bundle(
        ...
        reviewed = self.orchestrator.apply_bundle_review(
            ...
            [
                ...
                BundleReviewAction(draft_id=bundle.draft_ids[1], action="approve"),
            ],
        )
        self.assertEqual(reviewed.status, "approved")

        commit = self.orchestrator.commit_bundle(bundle.bundle_id, kind="add", confirmed_by="analyst-3")
        self.assertEqual(commit.committed_count, 2)
        self.assertIsInstance(commit.per_item_results[0], WriteResult)
        self.assertIsInstance(commit.per_item_results[1], WriteResult)

        claims = self.kg.query_claims(self.runtime_session_id, pred_id="user:tag")
        self.assertEqual(len(claims), 2)
        sources = {claim.meta["source"] for claim in claims}
        expected = {
            f"doc:{staging.source.doc_name}:seg:{staging.segments[0].segment_id}",
            f"doc:{staging.source.doc_name}:seg:{staging.segments[1].segment_id}",
        }
    ...
    def test_build_layer3a_tool_registry_exposes_forty_five_tools(self) -> None:
        ...
        self.assertIn("extract_from_segments", tool_registry)
        self.assertIn("extract_and_create_document_bundle", tool_registry)
