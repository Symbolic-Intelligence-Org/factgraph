...
- `sdk.check(...)` / `sdk.diagnose(...)` (G1) and `sdk.why_not(...)` (G4) are the L Direction SDK shell consumers of the capability helper package: they lower SDK `Derivation` inputs, call `build_check_request(...)` / `build_diagnose_request(...)` / `build_why_not_candidate_universe(...)`, and delegate to `check_derivation_binding(...)` / `diagnose_derivation_binding(...)` / `check_why_not_universe(...)` respectively. All three reuse `kernel.sdk.shells._validation.validate_derivation(...)` and remap non-SDK exceptions to `SDKStoreError(...) from exc` with capability-specific paths.
- Fact Overlay Check, ProofFrame Rechecker, Rule Disable, Rule Literal Replace, and the remaining capability ergonomics helpers are currently exposed at the application layer only. Any future SDK entrypoint must remain a thin delegate to `check_fact_overlay_binding(...)`, `recheck_proof_frame(...)`, `check_rule_disable_action(...)`, `check_rule_literal_replace_action(...)`, or the application helper functions. Frontier trace stays advanced importable per G4 §5.4 — the evaluator drift gate forbids application-layer or SDK-layer opt-in until a separate scoped blueprint reopens the boundary.
