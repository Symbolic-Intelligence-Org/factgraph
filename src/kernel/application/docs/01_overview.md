...
- `sdk.check(...)` / `sdk.diagnose(...)` (G1), `sdk.why_not(...)` (G4), and `sdk.check_fact_overlay(...)` / `sdk.recheck_proof_frame(...)` (G2) are the L Direction SDK shell consumers of the capability helper / application protocol packages. G1 + G4 lower SDK `Derivation` inputs and call `build_check_request(...)` / `build_diagnose_request(...)` / `build_why_not_candidate_universe(...)` before delegating to `check_derivation_binding(...)` / `diagnose_derivation_binding(...)` / `check_why_not_universe(...)`. G2 Fact Overlay reuses the same SDK `Derivation` lowering path and constructs `FactOverlayCheckRequest(...)` directly with a raw `EvaluationOverlay` (no A-side helper); G2 ProofFrame Recheck takes a raw `SupportArtifact` + raw `EvaluationOverlay` and constructs `ProofFrameRecheckRequest(...)` directly (no derivation lowering, no registry resolution, no engine arg). All five L methods reuse `kernel.sdk.shells._validation.validate_derivation(...)` (G1+G4+G2 Fact Overlay) and `validate_evaluation_overlay(...)` (G2 Fact Overlay + G2 ProofFrame Recheck), and remap non-SDK exceptions to `SDKStoreError(...) from exc` with capability-specific paths.
- Rule Disable, Rule Literal Replace, Rule Add Condition, Round events, ProofFrame diff, and the remaining capability ergonomics helpers are currently exposed at the application layer only. Any future SDK entrypoint must remain a thin delegate to `check_rule_disable_action(...)`, `check_rule_literal_replace_action(...)`, `check_rule_add_condition_action(...)`, or the corresponding application runtime / helper functions. Frontier trace stays advanced importable per G4 §5.4 — the evaluator drift gate forbids application-layer or SDK-layer opt-in until a separate scoped blueprint reopens the boundary.
