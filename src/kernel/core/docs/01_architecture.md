...
`src/kernel/core/store/ledger.py` 定义 append-only 数据结构：
...
- `kernel.adapters.souffle` import 时注册 `souffle`
- `kernel.adapters.problog` import 时注册 `problog`
- `kernel.adapters.pyreason` import 时注册 `pyreason`
...
`src/kernel/core/annotation/` 当前是 internal / prototype 落点。
