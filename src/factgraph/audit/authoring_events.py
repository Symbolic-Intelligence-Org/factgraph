...
def load_authoring_apply_events(registry_root: str | Path) -> list[AuthoringApplyEvent]:
    ...
    path = root / "authoring_apply_events.jsonl"
    if not path.exists():
        return []
    ...
    for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        ...
        try:
            ...
            raise AuthoringAuditReadError(f"invalid authoring apply event JSON at line {line_no}: {exc}") from exc
        if not isinstance(payload, dict):
            raise AuthoringAuditReadError(f"authoring apply event must be object at line {line_no}")
        events.append(
            AuthoringApplyEvent(
