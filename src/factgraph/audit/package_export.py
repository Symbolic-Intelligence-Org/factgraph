"""Engine-neutral audit-package export.

The audit ledgers (run / candidate / accept-write / decision / accept-failed /
mapping-resolution + optional support / rule-trace / certainty / provenance /
assertion-annotation artifacts) are derived ENTIRELY from the :class:`Store`. None of
the builders here touch the Souffle runtime, a Souffle binary, or any engine-specific
data structure: they read ``store.ledger`` / ``store.schema_ir`` / ``store.resolve_mapping``
/ ``store._support_artifacts`` / ``store._rule_trace_artifacts`` and pure-python helpers.

This module is the single source of truth for that logic. Two callers reuse it:

* :func:`export_audit_package` — the engine-neutral entry point. Writes an audit-only
  package (``manifest.json`` + ``audit/`` ledgers) that ``factgraph.audit.load_audit_package``
  reads back. It does NOT require Souffle, an ``ExportOptions`` with a Souffle
  ``target_engine`` default, or the facts/rules/policy codegen.
* ``factgraph.adapters.souffle.package.export_package`` — the existing combined
  inference/audit package. For ``package_kind="audit"`` it calls
  :func:`write_audit_artifacts` so the audit ledgers it emits stay byte-identical while
  the facts/rules/schema/policy path is unchanged. No duplicate ledger logic.

The builders were lifted verbatim from the Souffle adapter; their output is unchanged.
"""
...
def export_audit_package(
    ...
    *,
    ...
    """Write an audit-only package and return the path to its ``manifest.json``.

    Engine-neutral: needs no Souffle install and no engine codegen. The result is
    loadable by :func:`factgraph.audit.load_audit_package` and queryable via
    :class:`factgraph.audit.AuditQuery`. Read-only on ``store`` (the builders only read).
    """
    if not isinstance(store, Store):
        raise TypeError("store must be Store")
    if policy_mode not in {"edb", "idb"}:
        raise ValueError("policy_mode must be edb|idb")
    if not isinstance(target_engine, str) or not target_engine:
...
def write_audit_artifacts(
    ...
    *,
    ...
    """Write the ``audit/`` ledger files into ``audit_dir`` and return the audit_files map.

    The returned mapping (manifest ``paths.audit_files``) is keyed by ledger name and
    valued by the package-relative path. Behaviour and bytes are identical to the audit
    block that previously lived inline in the Souffle adapter's ``export_package``.
    """
    ...
    mapping_audit = _build_mapping_audit_payload(store, policy_mode)
    ...
    if provenance_statuses:
        provenance_status_rows = [
            {"candidate_id": candidate_id, **status_row}
            for candidate_id, status_row in sorted(provenance_statuses.items())
        ]
    ...
    if evidence_graphs:
        evidence_graph_rows = [
            {"candidate_id": candidate_id, "evidence_graph": graph}
            for candidate_id, graph in sorted(evidence_graphs.items())
        ]
