# Service layer current state (factgraph-core perspective)

- Scope: the dependency relationship between the HTTP/BFF delivery
  layer and `factgraph.core`
...
- Perspective: this document describes the service layer **from the
  ...
  (monorepo; not in the factgraph-only package).
...
Service sits above `factgraph.core` as the HTTP / BFF delivery facade.
...
- API key authentication (`X-factgraph-API-Key`)
...
own overview doc (monorepo; not in the factgraph-only package). This
...
Service routes go through stable entry points of `factgraph.core`;
...
| `POST /v1/runtime/sessions/{id}/writes/{set,add,retract}` | `factgraph.core.write_protocol.{set,add,retract}_write(...)` |
| `POST /v1/runtime/sessions/{id}/queries/view-facts` | `factgraph.core.store.queries.project_view_facts_with_audit(...)` |
| `POST /v1/runtime/sessions/{id}/rules/run` | `factgraph.core.rules.run_rule(...)` |
...
- maintaining a schema-digest cache at the service layer (owned by
  core's `Store`)
- consuming `factgraph.application.protocol.*` internal dataclasses
...
- `factgraph.application.protocol.common._validate_*` (protocol DTO
  validation)
...
- Service's own complete documentation: the service module's own
  overview doc (monorepo; not in the factgraph-only package)
- Core public contract:
  [`04_public_contract_v1.md`](./04_public_contract_v1.md)
- Application advanced-importable surface:
  [`src/factgraph/application/docs/01_overview_en.md`](../../application/docs/01_overview_en.md)
- Audit package contract:
  [`src/factgraph/audit/docs/03_audit_package_contract.md`](../../audit/docs/03_audit_package_contract.md)
- Batch 8 public-surface decision: archived `public-surface`
  blueprint inside the routemap (monorepo; not in the factgraph-only
  package)
...
- Post-routemap state (2026-05-06): the service layer has not
  ...
  `factgraph.application` / `factgraph.audit`).
