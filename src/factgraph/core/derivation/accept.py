...
def _resolve_candidate_key_to_entity_ref(
    *,
    ...
    refs: set[str] = set()
    for row in ledger._effective_meta_rows(key="candidate_key", kind="str"):
...
def _business_meta_for_duplicate(
    ...
    meta = {row.key: row.value for row in ledger._effective_meta_rows(asrt_id=asrt_id)}
...
def _meta_value(ledger: Ledger, asrt_id: str, key: str) -> str | None:
    for row in ledger._effective_meta_rows(asrt_id=asrt_id, key=key):
