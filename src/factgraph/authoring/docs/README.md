...
- The persisted declarative metadata contract is unified to `version /
  description / tags`, plus rule-scoped `condition_weights`; schema
  `repr` templates are validated for explain-layer use but remain out of
  compiled Schema IR in S1:
