...
- Signature:   ``diff_proof_frames(round_a_id, round_b_id,
               ...
               ``factgraph.audit.load_audit_package`` or hold them from
               a fresh recorder.
- Return:      ``ProofFrameDiff`` (raw application-canonical DTO from
               ``factgraph.audit.proof_frame_diff``; documented passthrough
...
- Errors:      Non-SDK exceptions crossing the SDK boundary remap to
               ...
               path (diff is pure ``factgraph.audit``, not a
               ``factgraph.application.capability_helpers`` builder).
...
above ``factgraph.core`` using ``factgraph.application.protocol``
vocabulary". Substrate IR (``factgraph.core.*``) remains excluded.
"""
...
def sdk_diff_proof_frames(
    ...
    *,
    ...
    ``factgraph.audit.proof_frame_diff.build_proof_frame_diff(...)`` 1:1.
    """
