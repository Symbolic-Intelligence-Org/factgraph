...
- Return:      ``CheckResult`` (raw application protocol DTO; documented
               ...
               ``factgraph.sdk.__all__``). For ergonomic evidence traversal,
               advanced callers can wrap ``result.evidence_envelope`` data
               with ``factgraph.application.walker.SupportArtifactView`` from
...
def sdk_check(
    ...
    *,
    ...
    can opt into ``factgraph.application.walker`` themselves.
