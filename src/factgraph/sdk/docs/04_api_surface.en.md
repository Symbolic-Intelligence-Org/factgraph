...
| Method | One-liner |
|---|---|
| `query(resolved_rule_or_policy, *, address_space=None)` | Start the native resolved Query builder. A `ResolvedRuleBundle` is lifted under alias `target`; a `Policy` requires its exact `SemanticAddressSpace`. `.bind(SemanticPortAddress, value).select(alias, SemanticPortAddress \| EvaluationQueryFieldNavigationV0).expect_contains(id, /, **selected_values).compile()/evaluate()` delegates to the existing compiled Query path. `.capture()` is the distinct terminal detached form: it returns `CapturedEvaluationQueryRunV0`, sealing an unmodified F4 bundle plus the ordered compiled contains-row inventory/outcomes; its `verify()` and positive-row `explain(...)` are detached. The navigation form is select-only: one structured identity-to-same-entity, single non-identity scalar-field lookup, Query-owned rather than `PolicyFieldNavigation`; missing field evidence yields no row. `.what_if(Q7_or_Q11_scenario).run()` is its terminal, captured `ScenarioRunV0` form. `expect_contains` observes only selected, schema-normalized result values and returns result-local `ExpectationResultV0` items; it is not a mode or product decision. It does not accept navigation in `bind`, bare Rules, string lookup, generic expect/modes, or Operators. |
