...
def _validate_rule_and_support(rule_spec: RuleSpec, support: ProofReceipt) -> None:
...
def _validate_rule_overlay_inputs(
    ...
    if overlay is not None and not isinstance(overlay, FactOverlay):
...
def _request_overlay(
    ...
    *,
    ...
    if overlay is None:
        return FactOverlay(rule_actions=(action,))
    if not isinstance(overlay, FactOverlay):
        raise CapabilityHelperError("overlay must be FactOverlay or None")
    if overlay.fact_actions or overlay.rule_actions:
        raise CapabilityHelperError(
            "non-empty overlay not supported; use overlay=None or FactOverlay()"
        )
    return FactOverlay(rule_actions=(action,))
