...
def _assertion_record_from_claim(
    ...
    *,
    ...
    meta = {row.key: row.value for row in ledger._effective_meta_rows(asrt_id=claim.asrt_id)}
...
def _claim_sort_key(ledger: Ledger, claim: Claim) -> tuple[int, bytes]:
    rows = ledger._effective_meta_rows(asrt_id=claim.asrt_id, key="ingested_at")
    ingested = -1
    if rows:
        row = rows[-1]
