...
class CompiledEvaluationQueryV0:
    ...
    def __post_init__(self) -> None:
        values = (self.query_digest, self.policy_digest, self.address_space_digest, self.schema_digest)
        ...
        if not isinstance(self.compiled_policy, CompiledPolicyV0) or not isinstance(self._lowering_plan, RuleExprLoweringPlan):
        ...
        if self.policy_digest != self.compiled_policy.policy_digest or self.address_space_digest != self.compiled_policy.address_space_digest:
        ...
        if self.query_digest != _digest(
            self.policy_digest, self.address_space_digest, self.schema_digest,
            self.bindings, self.selections,
        ):
...
def compile_evaluation_query(
    ...
    if not isinstance(compiled_policy, CompiledPolicyV0):
        raise _error("compiled_policy must be CompiledPolicyV0", "INVALID_COMPILED_POLICY", "query_compile")
    if not isinstance(address_space, SemanticAddressSpace) or not isinstance(schema_index, SchemaIndex):
        raise _error("address_space and schema_index must be trusted runtime values", "INVALID_QUERY_CONTEXT", "query_compile")
    ...
    for item in query.bindings:
        ...
        bindings.append(ResolvedEvaluationQueryBinding(
    ...
    query_digest = _digest(
        compiled_policy.policy_digest, address_space.address_space_digest,
        schema_index.schema_digest, normalized_bindings, normalized_selections,
    )
    ...
    if head.id in schema_index.predicates_by_id:
        raise _error(
            ...
            "QUERY_PROJECTION_NAMESPACE_COLLISION", "query_admission",
        )
    try:
        ...
        return CompiledEvaluationQueryV0(
            ...
            schema_index.schema_digest, normalized_bindings, normalized_selections, head,
            compiled_policy, plan,
        )
    except (PolicyError, RuleExprError, ValueError) as exc:
        raise _error(
            "query lowering rejected trusted compiled inputs",
            "QUERY_LOWERING_INVARIANT", "query_compiler_invariant",
            details={"cause_type": type(exc).__name__},
        ) from exc

def _assert_context(
    query: EvaluationQuery, policy: CompiledPolicyV0,
    space: SemanticAddressSpace, index: SchemaIndex,
) -> None:
    try:
        ...
        raise _error(
            ...
            "QUERY_POLICY_CONTEXT_STALE", "query_admission",
            details={"policy_code": exc.code},
        ) from exc
    if query.policy_digest != policy.policy_digest:
        raise _error("query does not reference this compiled Policy", "QUERY_POLICY_DIGEST_MISMATCH", "query_admission")
    if space.address_space_digest != policy.address_space_digest:
        raise _error("address space does not match compiled Policy", "QUERY_ADDRESS_SPACE_MISMATCH", "query_admission")
    ...
    try:
        for managed in space.occurrences:
            ...
            pin = pins.get(alias)
            pinned = None if pin is None else (
                pin.rule_id, pin.rule_version, pin.rule_content_digest, pin.semantic_contract_digest,
            )
            current = (rule.id, rule.version, rule.content_digest, managed.contract.semantic_contract_digest)
            if pinned != current:
                raise _error("compiled Policy Rule pins are stale", "QUERY_POLICY_CONTEXT_STALE", "query_admission")
            schema_digests.add(managed.contract.schema_digest)
    except SemanticPortResolutionError as exc:
        raise _error("managed Rule contract is stale", "QUERY_POLICY_CONTEXT_STALE", "query_admission") from exc
    aliases = {item.occurrence.alias for item in space.occurrences}
    if set(pins) != aliases or schema_digests != {index.schema_digest}:
        raise _error("compiled Policy and trusted schema context do not agree", "QUERY_SCHEMA_CONTEXT_MISMATCH", "query_admission")

def _branch_sources(
    policy: CompiledPolicyV0, address: SemanticPortAddress,
    ...
    for branch in policy.branches:
        try:
            ...
            raise _error(
                ...
                "PARTIAL_BRANCH_QUERY_ADDRESS", "query_admission",
                ("address", address.occurrence_alias, address.port_name),
                {"missing_branch_id": branch.branch_id},
            ) from exc
        sources.append(EvaluationQueryPortSource(
            branch.branch_id, branch.lowered_occurrence_aliases[offset], address.port_name,
        ))
...
def _resolve(address: SemanticPortAddress, space: SemanticAddressSpace) -> Any:
    try:
        ...
        raise _error(
            str(exc), "UNRESOLVED_QUERY_ADDRESS", "query_admission",
            ("address", address.occurrence_alias, address.port_name),
...
def _normalize_binding(
    ...
    try:
        if isinstance(endpoint, EntityIdentityEndpoint):
            if not isinstance(binding.value, EntityRef) or binding.value.entity_type != endpoint.entity_type:
                raise ValueError(f"expected EntityRef[{endpoint.entity_type}]")
            identity = materialize_identity(endpoint.entity_type, binding.value.identity, index=index)
            typed_value: Any = encode_entity_ref(EntityRef(endpoint.entity_type, identity), index=index)
        ...
        raise _error(
            ...
            "QUERY_BINDING_TYPE_MISMATCH", "query_admission",
...
def _query_value_bindings(bindings: tuple[ResolvedEvaluationQueryBinding, ...]) -> tuple[_RuleExprQueryValueBinding, ...]:
    return tuple(sorted(
        (
            _RuleExprQueryValueBinding(
            ...
            for item in bindings for source in item.sources
        ),
        key=lambda item: (item.branch_id, item.occurrence_alias, item.port_name),
    ))

def _error(
    message: str, code: str, stage: str, path: tuple[str, ...] = (),
