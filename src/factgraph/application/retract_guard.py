...
Per P1 #1 amend:
- Uses ONLY existing `Ledger.get_claim(asrt_id)` API — NO new Ledger helper
  added (preserves SF5 Q-PR1 carve-out / `core/store/ledger.py` 0 diff)
...
RetractClassification = Literal["identity", "exists", "unprotected"]
...
class RetractGuardError(Exception):
    ...
    Attributes:
        ...
        classification: Either "identity" or "exists".
    """
