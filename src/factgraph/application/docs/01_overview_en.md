...
- narrow ProofFrame Rechecker (`still_valid` / `invalidated` / `unknown` over one native `ProofReceipt` under fact replace/remove overlay, with deterministic single-frame narrative)
...
- `proofframe_runtime.py`
  - `recheck_proof_frame(...)`: rechecks one native `ProofReceipt` under an `FactOverlay` without re-running derivation evaluation; returns per-atom verdicts and aggregate frame status. Non-native support artifacts and rule-ref frames return frame-level `unknown`; `not` steps are strictly deferred to `unknown`.
