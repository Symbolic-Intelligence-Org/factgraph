...
def build_resolved_rule(
    *,
    ...
    rule = Rule(
        id=id,
        when=when,
        ports={name: port.var for name, port in declared.items()},
        version=version,
        repr=repr,
    )
...
def resolve_rule_contract(
    rule: Rule,
    semantic_ports: Mapping[str, SemanticRulePort],
    *,
    schema_index: SchemaIndex,
) -> ResolvedRuleContract:
    ...
    missing, extra = (
        sorted(set(rule.ports) - set(declared)),
        sorted(set(declared) - set(rule.ports)),
    )
    if missing or extra:
        raise _error(
            "semantic ports must exactly cover Rule.ports",
            "PORT_COVERAGE_MISMATCH",
            ("semantic_ports",),
...
def assert_rule_contract_current(rule: Rule, contract: ResolvedRuleContract) -> None:
    ...
    try:
        ...
        raise _error(
            "Rule content is no longer digestible",
            "RULE_CONTRACT_STALE",
            ("contract", "rule_content_digest"),
            {"cause_type": type(exc).__name__},
        ) from exc
    contract_ports = {name: port.var for name, port in contract.ports.items()}
    if (
        rule.id != contract.rule_id
        or rule.version != contract.rule_version
        or digest != contract.rule_content_digest
        or dict(rule.ports) != contract_ports
    ):
...
def _normalize_ports(ports: Mapping[str, SemanticRulePort]) -> dict[str, SemanticRulePort]:
    if not isinstance(ports, Mapping) or not ports:
        raise _error(
            "semantic ports must be a non-empty mapping",
            "INVALID_SEMANTIC_PORTS",
            ("semantic_ports",),
        )
    ...
    for name, port in ports.items():
        if not isinstance(name, str) or not name or not isinstance(port, SemanticRulePort):
            raise _error(
                "invalid semantic port entry", "INVALID_SEMANTIC_PORTS", ("semantic_ports",)
            )
...
def _validate_endpoint(
    ...
    try:
        ...
        if isinstance(endpoint, FieldEndpoint):
            ...
            if value_type.value_kind == "entity_ref":
                raise _error(
                    ...
                    "UNSUPPORTED_ENTITY_REF_FIELD",
                    ("semantic_ports", name, "endpoint"),
                )
            if rule.port_types[name].kind != "value":
                raise _error(
                    f"port {name!r} is not a scalar value",
                    "PORT_EXECUTION_TYPE_MISMATCH",
                    ("semantic_ports", name),
                )
        ...
        if isinstance(endpoint, FunctionValueEndpointV1):
            if rule.port_types[name].kind != "value":
                raise _error(
                    f"function port {name!r} is not a scalar value",
                    "PORT_EXECUTION_TYPE_MISMATCH",
                    ("semantic_ports", name),
                )
            _require_witness(
                rule.when,
                endpoint.relation_predicate_id,
                1 + len(rule.ports),
                endpoint.position,
                var,
                name,
        ...
        raise _error(
            str(exc),
            "SEMANTIC_ENDPOINT_NOT_FOUND",
            ("semantic_ports", name, "endpoint"),
            {"schema_error_code": exc.code},
        ) from exc
    raise _error(
        f"unsupported endpoint for port {name!r}",
...
def _require_witness(
    ...
    found = any(
        ...
        and atom.pred_id == pred_id
        and len(atom.terms) == arity
        and atom.terms[position] == var
        for atom in atoms
    )
    if not found:
        raise _error(
            f"port {port_name!r} has no positive {pred_id!r} witness",
            "SEMANTIC_VAR_POSITION_MISMATCH",
            ("semantic_ports", port_name, "var"),
        )
