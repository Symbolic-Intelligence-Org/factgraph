...
| `WhyNotUniverseResult.green` ([src/kernel/application/protocol/derivation_why_not.py:298](../../../../src/kernel/application/protocol/derivation_why_not.py)) | raw `tuple[BindingItems, ...]` | YES — first-slice attachment for generic `BindingsView` / `BindingView` |
| Future live store/ledger walker (real API TBD) | no source-backed `Store.active_facts` API today | **DEFER** — post-B3/future trigger; a real store/ledger source API must be named first |
| `DiagnoseAtomLocator.attempted_binding` ([src/kernel/application/protocol/derivation_diagnose.py:81](../../../../src/kernel/application/protocol/derivation_diagnose.py)) | raw tuple | YES — first-slice attachment for generic `BindingView` |
| `SupportArtifact.pred_witnesses` / `non_fact_steps` / `rule_ref_edges` | already frozen dataclass tuples | **NO DTO field walker needed**, but **YES** wrapper-view/cross-reference helpers (`SupportArtifactView`, `asrt_id → AssertionView` via frozen index, atom-key parsing helper) |
| `ProofFrameRecheckResult.atom_verdicts` | already frozen dataclass tuple | NO new walker class — covered by `ProofFrameView` per-DTO wrapper view (B2) |
| `ProofFrameDiff.frame_deltas` / `atom_deltas` | already frozen dataclass tuples | YES wrapper view — `ProofFrameDiffView` per-DTO wrapper view (B2). Common patterns (filter `frame_deltas` by status_change kind, flatten all `atom_deltas` across frames) require nested loops today; a wrapper view aligns with the same pattern as `SupportArtifactView` / `ProofFrameView`. **Does not modify the DTO and does not attach methods to its tuple fields.** Verdict revised by 2026-05-07 evidence-ecosystem audit (Lane 2 firm challenge). |
...
**Verification finding (Agent 2 2026-05-07):** What looked like "10+ objects need walker" is actually **2-3 objects need full walker classes** + **wrapper views over already-frozen dataclass tuples** (with `FrozenTupleView` / `frozen_collection(...)` as the standalone helper) + **cross-reference helpers** for SupportArtifact's atom-key format conventions and `asrt_id → fact tuple` lookup.
...
**Reconciliation with Evidence Graph (verified 2026-05-07):** `EvidenceGraph` is the audit layer's unified cross-engine explainability DTO. It normalizes engine-native provenance (Souffle/ProbLog/PyReason) into a shared node-edge-layout representation, durable as `audit/evidence_graphs.jsonl`, queryable via `AuditQuery.get_candidate_evidence_graph(candidate_id)`, and rendered via `render_evidence_graph_html()`. It is frozen and immutable; not a traversal abstraction. The walker mechanism (iterating IR / SupportArtifact atoms at application layer) operates on raw protocol data *before* EvidenceGraph construction. Walkers and EvidenceGraph are complementary, not overlapping.
