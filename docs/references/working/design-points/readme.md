...
- [factgraph-lifecycle-and-assets.zh.md](./factgraph-lifecycle-and-assets.zh.md) — Chinese working note on `FactGraph.create/load/save`, registry-as-backing-mechanism, domain asset namespaces for rules / inferences / queries, schema evolution boundaries, and evidence/audit placement.
- [query-view-and-inference-handles.zh.md](./query-view-and-inference-handles.zh.md) — Chinese working note on deferred Query/Search witness ids, frozen view as assertion-universe handle, and the handle boundary between Query/Search and Inference.
