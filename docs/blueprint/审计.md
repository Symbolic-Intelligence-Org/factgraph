...
* 同时写入撤销元信息（硬规范，revokes-only）：
  ...
  - 新增撤销事件并追加 `revokes(R1, A1).`（撤销模型唯一权威见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/core/断言层 证据层.md` 的 `写入协议：Field 操作 → claim/meta/active/chosen（MVP）`；禁止 meta_bool revoked 模型）。
...
硬约束：`policy_digest` 必须等于导出包 `manifest.json` 的 `digests.policy_digest`，其计算口径以 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/runtime/导出与运行.md` 的 `### 3.3 digest 口径（硬规范）` 为准；不允许自定义“组合 hash”。
