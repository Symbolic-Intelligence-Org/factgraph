...
- Verify whether existing SDKStore method naming has any namespace precedent.
- Verify whether a polymorphic method would violate `#3` by forcing a discriminated union across three distinct argument shapes.
- Verify whether three explicit methods create unacceptable outward surface noise under `#6`.
- Verify whether G3 should defer entirely because raw `RuleSpec` cannot be resolved safely in §5.2.
