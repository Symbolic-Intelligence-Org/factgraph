...
- 2026-05-05 (Step 0.B) — **D4 Result DTO freeze.** The result DTO is `WhyNotUniverseResult(status, requested_universe, green, red, errors, warnings)`. Top-level `status` is `completed`, `unsupported`, or `invalid_request`. `green` is a tuple of derived universe bindings. `red` is a tuple of `WhyNotRedRow`. Top-level non-completed statuses are reserved for request-wide failures before board assembly; per-row unsupported / invalid diagnostics stay on rows.

- 2026-05-05 (Step 0.B) — **D5 Row DTO freeze.** `WhyNotRedRow(binding, diagnostic)` carries the red binding and a `WhyNotRowDiagnostic`. `WhyNotRowDiagnostic` copies Diagnose's stable semantics: `status`, `matched_count`, `matched_binding`, `failure_kind`, `diagnostic_payload`, `errors`, and `warnings`. `diagnostic_payload` is `WhyNotAtomLocator | None`.
