...
- Do not route the shipped flat write methods (`fg.set`, `fg.add`, `fg.retract`, `fg.edit`, `fg.ingest`, `fg.add_schema_classes`, `fg.save_rule`, `fg.save_inference`) through `Database.commit_assertions(...)` on attached SDKStores.  Reject them instead;refactoring those paths to be Database-routed is a separate later slice (Q1 boundary expansion).
...
- Flat: `fg.set(...)`, `fg.add(...)`, `fg.retract(...)`, `fg.edit(...)`, `fg.ingest(...)`, `fg.add_schema_classes(...)`, `fg.save_rule(...)`, `fg.save_inference(...)`, `fg.save(...)`.
...
Reads (`fg.read.find`, `fg.read.get`, `fg.read.ref`, `fg.assertions.by_id / by_ids / active / all / field`, `fg.eval.*`, `fg.what_if.*`, `fg.audit.*`, `fg.package.*`, `fg.rules.inspect / load / list / get`, `fg.inferences.load`, `fg.views.get / list`) work unchanged on attached SDKStores — they query the `Ledger` shared with `db` and observe the same data.

Rationale for "reject" rather than "route through Database":

The Database-routed alternative would be to rewrite each shipped flat write method (`fg.set`, `fg.add`, etc.) so that on attached SDKStores it internally constructs an `AssertionInput` and calls `db.commit_assertions(...)`.  That refactor touches every write path (~9 flat methods plus their manager delegates plus shared helpers like `_SDKWriteManager`) and changes the contract for what an assertion id looks like at the SDK boundary (slice-1 content-addressed `asrt_id` vs the shipped path).  It also reopens questions about meta handling, ingest validation, and schema mutation rejection that span far beyond the Q2 attach lifecycle.  Per `feedback_smaller_batch_design_blueprints`, that work is its own slice;this slice defers it by rejecting rather than re-routing.
...
- All shipped flat write methods (`fg.set`, `fg.add`, `fg.retract`, `fg.edit`, `fg.ingest`, `fg.add_schema_classes`, `fg.save_rule`, `fg.save_inference`, `fg.save`) work unchanged.
- All shipped manager methods (`_SDKWriteManager.*`, `_SDKSchemaManager.*`, `_SDKRulesManager.*`, `_SDKInferencesManager.*`, `_SDKViewsManager.*`) work unchanged.
...
- [ ] On attached SDKStores, the following flat methods raise `SDKStoreError`: `fg.set`, `fg.add`, `fg.retract`, `fg.edit`, `fg.ingest`, `fg.add_schema_classes`, `fg.save_rule`, `fg.save_inference`, `fg.save`.
- [ ] On attached SDKStores, the following manager methods raise `SDKStoreError`: `_SDKWriteManager.set / add / retract / edit`, `_SDKSchemaManager.add / ingest`, `_SDKRulesManager.save`, `_SDKInferencesManager.save`, `_SDKViewsManager.create / update / delete`.
