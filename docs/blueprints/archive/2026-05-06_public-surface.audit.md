...
| 2026-05-06 | implemented | Implementation completed | Implemented docs/checklist-only Batch 8:README / README.en now state product public vs advanced importable vs out-of-package surfaces;SDK API docs state no Batch 3-7 SDK shells;application/audit docs label their surfaces as advanced importable;release-candidate blueprint gained a Batch 8 public-surface checkpoint. Verification:projection script passed(261 files),README quickstarts print `Alice`,public-doc deny-pattern scan clean,`git diff --check` clean,and drift gates show no Python code,release script,allowlist,`pyproject.toml`,service,agent,domain,or release branch changes. |
