...
| 2026-05-06 | implemented | Post-archive hardening | Batch 5c strict review found the same shallow RuleRef body scan in all three rule-action runtimes. Rule Literal Replace now recursively detects `ruleref` inside compound atoms currently represented by `not` bodies,so `not(ruleref(...))` maps to `RULE_LITERAL_REPLACE_RULE_REF_UNSUPPORTED` instead of leaking through as `RULE_LITERAL_REPLACE_NATIVE_EVAL_ERROR`. Added a focused regression without changing protocol surface or RuleRef substrate. |
