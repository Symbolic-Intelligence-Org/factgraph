...
| 2026-05-11 | implementing | G3 docs synced | Updated release-facing SDK, adapter, authoring, and core docs to stop teaching public `Rule(..., engine_ext=...)` / `Derivation(..., engine_ext=...)` syntax. Remaining `engine_ext` mentions are limited to public rejection, adapter/internal bridge, or future SemanticsProfile migration contexts. Stale-syntax grep gate is clean. |
| 2026-05-11 | implemented | Closed and archived | Filled §10 Outcome / Deviations, marked the blueprint implemented, moved the blueprint pair to archive, and updated archive inventory. |
