> 状态：已实现（v1 主线）  
> 类型：协议蓝图  
> 说明：本文档保留为历史 SchemaIR / identity / canonical 规范；其 v1 主线结论已进入当前实现，但当前真相仍以相关模块 docs 为准。
...
* 下游约束：禁止运行时推断/补字段；type_domain 与 Tag 枚举对齐（见 `/Users/zhenzhili/hnsm-backend/docs/断言层 证据层.md` 的 `typed_tuple_v1 与 claim_arg：编码与 Tag 枚举（硬规范）`）
