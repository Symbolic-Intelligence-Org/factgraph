...
owner: runtime/export
last_verified: 2026-03-14
---

> Partial status: 本文定义 Exporter/Runner 的目标语义与包契约，但具体导出产物与当前 adapter 行为仍需结合现行实现文档复核。

# 导出与运行（Exporter/Runner）规范（MVP）

本文件定义 Exporter/Runner 的**唯一权威**导出与执行协议（MVP）。除本文件明确声明的扩展点外，任何实现不得引入“隐式降级/近似”导致跨引擎不可复现。

适配器层（含引擎 evaluate）在 Derivation 输入语法上与 v2 主链路保持一致：`head` 决定 candidate kind（`EntityType(...) -> entity`，`Entity.field(...) -> fact`），用户侧不再使用 `materialize_as/id_policy`。
...
* `schema/schema_ir.json` 的结构定义见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/schema/规范.md` 的 `SchemaIR：结构（JSON Schema）与 schema_digest（硬规范）`；policy(active/chosen) 与 mapping 的语义约束见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/core/视图层.md` 的 `## 6. Policy：active / chosen（默认与约束）` 与 `## 8. Mapping（canon_of 等）视图`；本文件只规定导出位置与 digest。
...
* EntityRef 规则对齐（见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/schema/规范.md` 的 `IdentityPolicy 与 EntityRef 生成（硬规范）`）
* `typed_tuple_v1/claim_arg` 规则对齐（见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/core/断言层 证据层.md` 的 `typed_tuple_v1 与 claim_arg：编码与 Tag 枚举（硬规范）`）
* 写入协议与撤销模型对齐（见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/core/断言层 证据层.md` 的 `写入协议：Field 操作 → claim/meta/active/chosen（MVP）`）
