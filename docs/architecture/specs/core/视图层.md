...
> Partial status: 本文可作为 view 语义基线，但实现细节与对外接口需结合当前 `core.view` 和 service/runtime 文档核对。
...
硬约束（MVP，必须）：`active(A)` 的语义仅由证据层“撤销模型 `revokes/2`”决定（见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/core/断言层 证据层.md` 的 `写入协议：Field 操作 → claim/meta/active/chosen（MVP）`）；禁止以 `meta_bool(A,"active",false)` 等 meta 模型替代。
