...
> Partial status: 本文可作为断言/证据语义的规范性参考，但具体导出、审计与 service 契约应以对应当前文档为准。
...
最终约束：`accept` 写回协议（见 `/Users/zhenzhili/symbolic_agent/docs/history/design-evolution/规则.md` 的 `accept 写回协议（MVP）与 Store API（最小合约）`）必须复用本节的撤销模型与幂等键策略，禁止另起一套。
...
- `entity_ref`
    ...
    - 必须符合 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/schema/规范.md` 的 `IdentityPolicy 与 EntityRef 生成（硬规范）`；禁止 pretty-display。
