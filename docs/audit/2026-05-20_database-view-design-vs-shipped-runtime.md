...
Q1-Q5 form a coherent dependency stack rather than 5 independent decisions. Resolution order is forced by data dependency: **Q1 (Database class) → Q3 (tx_id primitives) → Q2 (attach lifecycle) → Q4 (FrozenAssertionView shape) → Q5 (view ↔ is_active composition)**. Until Q1-Q5 are answered, I1-I9 cannot move past **(c) shape conflict** / **(d) genuinely new** / **(e) deferred-aligned** classification into concrete implementation recommendations.
