...
| A11 | branch / writable sub-fg / remote / multi-db / Rule persistence 全部 deferred | A11 is a **deferred-items meta-row** — verifies shipped doesn't accidentally implement any of the 5 listed items as **Database/view-substrate features**. Per-item verification: **branch / writable sub-fg / remote / multi-db**: no shipped concept anywhere (verified by §7 inventory absence — none of these surface in any of the 17 read files). **Rule persistence**: shipped HAS rule persistence via `FileAuthoringRegistry` (§7.12 `sdk/store.py:save_rule:2092` / `load_rule:2105` / `list_rules:2113` / `get_rule:2120` → `application.authoring_runtime`). **Interpretation** (per design §17 D9 cross-reference + A15 rules-as-code framing): A11 means "**Rule persistence as a Database/view v1 substrate feature** is deferred"; shipped `FileAuthoringRegistry` is a separate SDK-layer concern **outside** the v1 Database/view design scope. A11 + A15 (revised) jointly state "rules are NOT part of Database/view v1 substrate; they are a separate concern". Shipped's `FileAuthoringRegistry` is consistent with this (separate concern), though shipped's DEFAULT placement of `registry/` inside workspace conflicts with A15's strict prohibition (per revised Q6, separate question). | **(a) shipped covers** — none of the 5 deferred items are implemented as Database/view-substrate features in shipped. Rule persistence at `FileAuthoringRegistry` is at a layer **outside** the v1 Database/view scope, so A11's deferral holds for the substrate. Caveat: this interpretation depends on Q1 (Database boundary) — if Q1 narrows "Database substrate" definition, A11 alignment narrows correspondingly. | low | confirm during Q1 resolution that "Database substrate" boundary excludes `FileAuthoringRegistry`. **Orthogonal to Q6** (registry placement migration) — Q6 is about where rules go within shipped's authoring layer; A11 is about whether rules are part of Database substrate at all. |
...
| A15 | rules 不进 workspace、不进 Database transaction;`FactGraph.attach(...)` 签名不含 `rules=`;`rule_set_digest` 在 evaluate 时计算 | This is the **design's authoritative anchor for rules-as-code governance** — referenced by revised Q6 (per commit `682f23eb`) and by I12. The audit-row text uses the design doc abbreviation; full design wording (§18 A15 of `database-view-fg-layered-architecture.zh.md:734`) reads: "rules 是 code artifact,v1 不进 workspace、不进 Database transaction;`FactGraph.attach(...)` 签名不含 `rules=`;rules 通过 `Expr / Inference / Rule` 内部引用承载,`rule_set_digest` 在 evaluate 时计算并写入 EvaluateResult metadata". Six sub-assertions verified separately. **(A) rules 是 code artifact**: SDK `Rule` / `Inference` objects are constructed in user code (§7.12 referenced via `_compile_rule_input` / `_compile_derivation_input`); `RuleRegistry` is in-memory dependency tracker; `FileAuthoringRegistry` persists rule artifacts as code-shaped JSON (not as Database assertions). Shipped largely aligns with rules-as-code philosophy. **(B) v1 不进 workspace**: **per revised Q6 + I12 (E) (commit `682f23eb`), shipped default violates strict prohibition** by auto-coupling `registry_root = workspace_path / "registry/"` (§7.12 `sdk/store.py:667-695`); `SDKStore.save()` rebinds `_authoring_registry` to `paths.registry` (§7.12 `sdk/store.py:2063-2090`). **(C) v1 不进 Database transaction**: per I12 (D) — rules go to `FileAuthoringRegistry`, never to `Ledger.append_*`. **(D) `FactGraph.attach(...)` 不含 `rules=`**: per I12 (A) — `attach()` doesn't exist; vacuously aligned. **(E) rules 通过 `Expr / Inference / Rule` 内部引用承载**: per I12 (B) — `_register_rule_dependencies` (§7.12 `sdk/store.py:2421-2451`) + `RuleRegistry` mechanism carries dependency references. **(F) `rule_set_digest` 在 evaluate 时计算 + 写入 `EvaluateResult` metadata**: per I12 (C) + I10 — `rule_set_digest` literally absent from shipped (grep-verified, 0 matches); `EvaluateResult` metadata seam is part of evidence service redraft (deferred, do NOT re-expand here). | Six-way split classification. **(A) (a) shipped covers** — rules-as-code is a shipped philosophy via SDK Rule/Inference objects. **(B) (c) shape conflict** [tightened 2026-05-20 per commit `682f23eb`; previously (c)+(a) split] — shipped default behavior violates design strict prohibition; migration mechanics per revised Q6. **(C) (a) shipped covers** — verified via I12 (D). **(D) (e) deferred-aligned vacuously** — attach lifecycle absent. **(E) (a) shipped covers in spirit** — exact-shape comparison vs design wording requires rule-expression doc audit (secondary surface, out of scope). **(F) (d) genuinely new + cross-doc seam** — `rule_set_digest` absent; metadata placement part of evidence service redraft per I10. | medium (mostly aligned in spirit; (B) is the tightened conflict requiring Q6 migration; (F) is cross-doc deferred per I10) | split recommendations: **(A) (C) (E)** accept **(a)** — shipped aligned. **(B)** design prohibition established; migration mechanics per revised Q6 (per commit `682f23eb`). **(D)** confirm in I4 / Q2 acceptance gate. **(F)** defer per I10 — cross-doc seam blocked on evidence service redraft + Q1-Q5. A15 is the most heterogeneous A-row in Phase 3 — 6 sub-assertions span (a) × 3 + (c) × 1 + (d) × 1 + (e) × 1. |
...
**A11-A15 cross-cutting observation** (informational, not a triage decision):

A11-A15 cover the **identity / shape / rules-governance commitments** — the most heterogeneous batch in Phase 3. Per-row character:

- **A11** is a **deferred-items meta-row** mirroring §5 / D-series style. 4 of 5 listed items (branch / writable sub-fg / remote / multi-db) absent everywhere; the 5th (Rule persistence) is implemented in shipped at `FileAuthoringRegistry` — but **outside the Database/view v1 substrate** per A11+A15+§17 D9 joint reading. Aligned **conditional on Q1** narrowing "Database substrate" boundary appropriately.
...
- **A15** is the **most heterogeneous A-row in Phase 3** — 6 sub-assertions across 4 classifications: (a) × 3 + (c) × 1 + (e) × 1 + (d) × 1. Per revised audit (commit `682f23eb`), sub-assertion (B) "v1 不进 workspace" is the tightened conflict — shipped default violates strict prohibition; migration via revised Q6.

Batch 3 classification distribution: **(a) shipped covers: 1 + 3 sub-assertions** (A11, A15 A/C/E); **(c) shape conflict: 1 + 1 sub-assertion** (A13-A, A15-B); **(d) genuinely new: 3 sub-assertions + 1 reinforcement** (A12 A/B/C, A13-B reinforces A3); **(e) deferred-aligned vacuously: 2 + 1 sub-assertion** (A14, A15-D). **1 new Q raised**: **Q7 (AssertionRecord shape reconciliation)**, parallel to Q6 (independent of Q1-Q5 chain). A11/A14 confirm-during-Qx;A12 defers to Q1+Q3;A13 defers to new Q7;A15 split — (A)(C)(E) accept, (B) migration via Q6, (D) confirm in Q2, (F) cross-doc seam deferred per I10. No implementation paths proposed.
...
Q1-Q5 form a coherent dependency stack rather than 5 independent decisions. Resolution order is forced by data dependency: **Q1 (Database class) → Q3 (tx_id primitives) → Q2 (attach lifecycle) → Q4 (FrozenAssertionView shape) → Q5 (view ↔ is_active composition)**. **Q6 (registry-in-workspace migration) is parallel** and independent of this chain — design prohibition is established (per A15 strict prohibition), only migration mechanics remain open. **Q7 (AssertionRecord shape reconciliation) is parallel** and independent of both the Q1-Q5 chain and Q6 — it touches record-shape layering, not db/view substrate identity or rules governance. Until Q1-Q5 are answered (Q6 + Q7 separately), I1-I13 cannot move past **(c) shape conflict** / **(d) genuinely new** / **(e) deferred-aligned** classification into concrete implementation recommendations.
