...
* Authoring 概念（Identity / Field / fact_key / reify）到 canonical SchemaIR/Rule 契约的映射见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/authoring/Authoring 层契约.md`。
...
- Core Ledger 已落地为 SQLite write-through cache：SQLite 持久化真相，内存索引承担读缓存。
- 写入侧已统一到原子入口：`append_assertion(...)` / `append_revocation(...)`。
- `write_protocol` 已切换到上述原子入口，`ingest_key` 幂等由 Ledger 内部负责。
- 查询接口对上层保持不变，`policy` / `view` / `sdk` 调用方无需感知后端变化。
- `SDKStore.from_schema_classes(..., ledger_path=...)` 已落地：可恢复 file-backed Ledger，并在首次创建时写入 `schema_digest`、后续恢复时执行一致性校验。
...
* Exporter/Runner 的唯一权威规范见 `/Users/zhenzhili/symbolic_agent/docs/architecture/specs/runtime/导出与运行.md` 的 `导出与运行（Exporter/Runner）规范（MVP）`。
