"""Tests for Diagnose capability helper builders."""
...
class BuildDiagnoseRequestTests(unittest.TestCase):
    def test_builds_diagnose_request_from_mapping_binding(self) -> None:
        plan = _plan()

        request = build_diagnose_request(plan, {"$age": 30, "$p": "person-1"})

        self.assertIsInstance(request, DiagnoseRequest)
        self.assertIs(request.plan, plan)
        self.assertEqual(request.binding, (("$age", 30), ("$p", "person-1")))
        self.assertEqual(request.engine, "native")

    def test_builds_diagnose_request_from_binding_items(self) -> None:
        binding = normalize_binding_items((("$p", "person-1"), ("$age", 30)))

        request = build_diagnose_request(_plan(), binding, engine="problog")
    ...
    def test_empty_binding_and_none_values_are_allowed(self) -> None:
        empty_request = build_diagnose_request(_plan(), {})
        none_request = build_diagnose_request(_plan(), {"$p": None})
