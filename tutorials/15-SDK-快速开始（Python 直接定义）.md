...
> - Rule / Derivation 的 runtime builder 还未实装；本教程对 registry 注册部分使用 payload dict（或继续使用 CLI/DSL）
...
### 注册 Rule / Derivation（当前用 payload dict）
...
> 说明：runtime `Rule/Derivation` builder 还未实装；当前先用 payload dict（或继续使用 CLI/DSL 文件）。
...
- ✅ 已有
  ...
  - `SDKRegistry`（apply schema classes、registry 只读、注册 rule/derivation payload）
- ❌ 还没有
  - runtime `Rule` / `Derivation` builder（对象式 API）
