# Audit Pillar Governance

Governs the `workflow/audit/` pillar — drift / anti-drift records. Codifies the rules locked by Q3 (commit `ae375ce5`).
...
|---|---|---|---|---|
| **Paired blueprint audit log** | `<basename>.audit.md` (sibling to blueprint) | `workflow/blueprints/{active,archive}/` | Per-blueprint event log of state transitions + decision notes | `workflow/blueprints/AGENTS.md` |
| **Standalone audit record** | `YYYY-MM-DD_<topic>-<subtype>.md` | `workflow/audit/{active,archive}/` | Cross-cutting drift triage, preflight safety check, post-Q synthesis | this AGENTS file |

This document governs the **standalone audit record** concept. For the paired blueprint audit log, see `workflow/blueprints/AGENTS.md`.
...
4. Cleanup-style slice (per `workflow/blueprints/AGENTS.md` cleanup-slice section).
5. Tiny local fix (per `workflow/CADENCE.md` Scope-and-Applicability).
...
- `vs-shipped.md` — Stage 1 audit template
- `preflight.md` — Step 4.3 preflight template (includes trigger condition check)
- `synthesis.md` — Stage 3 synthesis template (includes 5-bucket structure)

Manual drafting (not from template) is discouraged; see `workflow/templates/README.md` §customization policy.

## See also

- `workflow/blueprints/AGENTS.md` for paired blueprint audit log convention (the other "audit" concept)
- `workflow/CADENCE.md` Stage 1 / Step 4.3 / Stage 3 for how the 3 sub-types integrate with the broader cadence
