# Design-Point: Meander × FactGraph 统一设计候选 v0.1.1（Option 1 text convergence / narrow lineage revision）
...
- Inputs:
  ...
  - Product research package — research evidence, not authoritative design:
    - [`01_执行摘要与最终判决.md`](</Users/zhenzhili/obsidian_workspace/symb-Intelli./codex_report/01_执行摘要与最终判决.md>), SHA-256 `cc6eb3297a4b33ceebbd675eb896b13519a2f183758ca1a03b7dd9a5acc65f3d`, anchor `:154-168`
    - [`09_实验路线_停止条件与迁移.md`](</Users/zhenzhili/obsidian_workspace/symb-Intelli./codex_report/09_实验路线_停止条件与迁移.md>), SHA-256 `7f8f73ce18b8e688daa485cf465a06157993a94bef60ed3b4a03f406df2da474`, anchors `:17-30/:98-106/:124-132/:220-225`
    - Additional context: [`06_Agent四级参与与Verify_Decide设计.md`](</Users/zhenzhili/obsidian_workspace/symb-Intelli./codex_report/06_Agent四级参与与Verify_Decide设计.md>), [`08_推荐架构与实施级MVP规格.md`](</Users/zhenzhili/obsidian_workspace/symb-Intelli./codex_report/08_推荐架构与实施级MVP规格.md>)
...
- Outputs / Downstream:
  - completed independent cold-start adversarial review: architecture `REVISE`; product authorization `STOP-except-discovery`; this v0.1.1 is only the Option 1 text-convergence lineage revision
...
> **Authority reminder**: this v0.1.1 is a narrow lineage revision of the frozen v0.1; its lifecycle relation is `not formal supersedes`. It is not an adopted architecture and not current product behavior. It does not modify, move, archive or formally supersede the frozen v0.1; supersede its input essays; change either repository; authorize a blueprint, product construction, experiment or implementation; or reopen/close an ADR. Its purpose remains to give an independent reviewer one coherent, falsifiable design instead of asking the reviewer to reconstruct intent from conversation history.
...
> If Meander continues beyond discovery, what is the smallest coherent review map for the stated scope, centered on a source-bound case or draft, in which Agent output or intent is one possible input, Meander resolves authority and product context, FactGraph performs deterministic query/scenario evaluation, and an operator can inspect the exact meaning, evidence, source, result row, and counterfactual without a second evaluator?
...
Gate -1 is the entry gate for Phase 0 and Phase 1. Only pure design and audit may continue without it; any exception requires separately named user authorization, is budget-capped and disposable, is not product approval, and must not force product continuation.

## 1. Executive design thesis

### 1.1 Product core — `PROVISIONAL` / `EXPERIMENT REQUIRED`; Gate -1 has not passed

Meander's defensible product core is:

> A source-bound case or draft Verify and review layer in which Agent output or intent is one possible input to a versioned, challengeable, replayable assessment under operator-owned policy and evidence rules.
...
Query and What-if are enabling capabilities. They become product surfaces only where they help a reviewer understand or challenge a source-bound case or draft.
...
### 1.3 Initial implementation cell — `PROVISIONAL` post-gate Agent/compatibility profile

Agent participation, Agent policy authority, and disposition are three axes rather than one ladder. The deliberately narrow post-Gate -1 Agent/compatibility profile is:
...
Parallel experiments are post-Gate -1 by default. The only exception is the §0.4 separately named, budget-capped, disposable path; it is not product approval. A1 Agent-selected published Policy, A2 inline Policy composition, L2 repair, and action enforcement remain gated.
...
| Governance gate precedes accepted-plan assertion writes | [`ingest.py:1078-1095`](</Users/zhenzhili/meander-latest-stack/meander/src/meander/plans/ingest.py:1078>) and [`ingest.py:1140-1169`](</Users/zhenzhili/meander-latest-stack/meander/src/meander/plans/ingest.py:1140>) |
...
| Current Rule, occurrence and port substrate | [FactGraph `rule.py`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/protocol/rule.py:52>), [`RulePortRef:218`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/protocol/rule.py:218>), [`RuleOccurrence:238`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/protocol/rule.py:238>) and [port inference](</Users/zhenzhili/hnsm-backend/src/factgraph/application/protocol/rule.py:557>) |
...
| Current Explain paths are not uniformly frozen | manual Explain re-evaluates against the current attached Store/Ledger ([`store.py:3020-3045`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3020>)); builder dispatch is [`:3367-3381`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3367>). The Soufflé preferred live-store path passes `self._store` at [`:3414`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3414>) within [`:3413-3424`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3413>), with captured-artifact/minimal fallback at [`:3427-3438`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3427>). The ProbLog preferred live-store path passes `self._store` at [`:3454`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3454>) within [`:3453-3469`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3453>); its captured proof trace path at [`:3472-3501`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3472>) injects a current-ledger certainty callback at [`:3488`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3488>) / [`:3518-3536`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3518>) and is therefore a hybrid fallback. Native current-ledger projection occurs at [`:3575`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3575>) within [`:3565-3595`](</Users/zhenzhili/hnsm-backend/src/factgraph/sdk/store.py:3565>). This evidence supports neither an all-path-live nor a fully-frozen/replay-safe claim. |
...
| Readonly RuleStructure/EvidenceGraph substrate | [FactGraph `EvidenceTimeline:143`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/explain/evidence_tree.py:143>), [`EvidenceGraph:155`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/explain/evidence_tree.py:155>), [`RuleStructure:229`](</Users/zhenzhili/hnsm-backend/src/factgraph/application/protocol/rule_structure.py:229>) and the [adopted readonly ADR](../../decisions/active/2026-06-26_evidencegraph-readonly-and-rule-structure-type.md) |
...
Meander fills the GovernanceSnapshot, required EvaluationUnit(s), projections/expectations, combining profile and execution profile from the assigned Plan contract. It is a post-gate Agent/compatibility profile, not the default MVP.
...
SHIPPED ingress and governance state machine
  -> governance-blocked: audit state; no parse/write/evaluation
  -> validation-rejected: audit state; no graph write/evaluation
  -> PlanWriteFailure + successful rollback: terminal rejected_write_failure; revoked traces may remain; no effective Claim state; no evaluation
  -> second WriteBoundaryError: terminal rejected_write_failure; no write; no evaluation
  -> unexpected post-create-ingesting failure: nonterminal ingesting; recovery-needed; not limited to rollback failure
...
A recovery-needed `ingesting` state may temporarily coexist with written facts; it is not a terminal branch.
...
Meander PlanStore owns Plan lifecycle records. The Meander graph service performs write, rollback, and recovery orchestration; the adjacent FactGraph ledger stores assertion/retract history. The target hnsm FactGraph and `LegacyV3Adapter` do not own the shipped Plan lifecycle and do not write, do not revoke, and do not reject Plan Claims.
...
flowchart LR
  ...
  B -->|"governance blocked"| X1["Audit state; no parse/write/evaluation"]
  B -->|"validation rejected"| X2["Audit state; no graph write/evaluation"]
  B -->|"rolled-back write failure"| X3["Terminal rejected_write_failure; no effective claims/evaluation"]
  B -->|"no-write boundary failure"| X4["Terminal rejected_write_failure; no write/evaluation"]
  B -->|"unexpected post-create failure"| X5["ingesting; recovery-needed"]
...
### 16.4 Source retention and privacy — `SETTLED DIRECTION`

The settled direction is limited to minimum disclosure and Meander access control. Unsettled: tenant keying remains open; retention/erasure/legal-hold/replay precedence remain open.
...
### 16.5 PolicyQueryContract as an information boundary — `PROVISIONAL`
...
### 16.6 TOCTOU and Decision isolation — Verify isolation `SETTLED DIRECTION`; future Decision profile `DEFERRED`
...
This remains a risk-first candidate sequence, not an implementation authorization. Gate -1 applies to Phase 0 and Phase 1; any separately named, budget-capped, disposable exception is not product approval. Phase 2 and every later production change still require D01 product gate evidence, the applicable adopted ADR(s), a scoped blueprint/preflight under repository governance, and explicit user authorization. Passing the local phase gate alone is insufficient.

Each authorized phase is bounded by its listed artifacts/deliverables and does not authorize the full review map.
...
Run 8–10 target-account walkthrough sessions and obtain ≥5 completed walkthroughs. Separately assemble one lawful, source-aligned historical case bundle covering 5–10 manual and native claim classes.

Compare existing QA, simple rules, an LLM judge, and FactGraph. Quote a standalone concierge and evaluator plugin as experimental arms, while retaining OEM and both/neither; this does not decide PM-03. Remain `STOP-except-discovery` until reviewer value, data access, and a cost-bearing pilot are evidenced.
...
**Entry:** Gate -1 must have passed, except only through the separately named §0.4 path.
...
3. **Phase 2 Managed Translator:** stop when customer mapping/ontology implementation and maintenance consumes `>50%` of the workflow effort.
...
The following preserve distinct promotion and total-stop scopes from the research package; they are not one global threshold:

- **Phase 3 promotion target, not a global kill threshold:** at least one customer-co-defined, payment-relevant result improves 30% relative to baseline.
- After 10 priced offers that include integration commitment, `<3 paid pilot` commitments is an independent total-stop condition.
- `>7 incumbent-sufficient buyers` is a separate total-stop condition.
- **Gate C / total stop:** semantic customization/engineering is `>50%` and cannot form a reusable vertical pack.
...
|---|---|---|---|
| D01 | Product wedge and product gate evidence | Determines whether this remains internal technology or a product | Phase 0 / Phase 1 and any product build beyond §0.4 discovery-parallel work; Gate -1 definition of done; does not decide PM-03 |
...
REVISE     promising, but phase-specific required revisions/gates must close at their assigned gate
...
## 23. v0.1.1 Option 1 lineage status note

This narrow lineage revision records the smallest coherent review map for the stated scope. It does not formally supersede the frozen v0.1. The completed adversarial review leaves these exact 16 findings open as `NEEDS_DECISION`:
...
The review map continues to cover:
...
The completed adversarial review did not close those 16 decisions or validate Plan generation, Translator fidelity, Policy compiler semantics, historical replay, interactive UI comprehension or market demand. It returned architecture `REVISE` and product authorization `STOP-except-discovery`; unresolved work remains with its assigned decisions/gates/experiments. Option 1 only converges current text and does not claim that validation passed or authorize an experiment or implementation.
