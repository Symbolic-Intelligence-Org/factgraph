...
| ID | 范围 | 性质 | 备注 |
|---|---|---|---|
| **T1.1** | 引入新 Rule 类(additive;与现行 `Rule(select, where, head)` **共存**,新类名待定 — 可能 `kernel.application.protocol.RuleV2` 或类似)+ unified atom canonical IR + lowering 复用 `ExistsAtom`/`AttrRef`/`CompareExpr`(§3.5 F6 锁定的 ~50-75 行 lowering 扩展)| Additive | T2/T3/T4/T5 的 prerequisite |
| **T1.2** | 拒绝 legacy 2-line 形态 + `Pred(...)` + 裸 AttrRef 比较(alpha 直接 hard-cut,无 transition)| Subtractive | 走 Step 4.6.5 pre-impl grep;blast radius 主要在 tests + sdk dsl |
