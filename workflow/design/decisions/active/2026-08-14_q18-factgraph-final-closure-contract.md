...
| 2026-08-14 | portable-parity correction | Soufflé occurrence-sensitive support reconstruction repaired | A final real cross-entity `PolicyFieldNavigation` / `PolicyCompare` probe found that same-predicate witnesses were collapsed by predicate id before support bindings were reconstructed. The adapter now preserves predicate-occurrence identity through reconstruction, then emits its compatible receipt view. Native, Soufflé and ProbLog agree on the regression's selected rows; repeat result: 688 tests and 175 subtests. |
