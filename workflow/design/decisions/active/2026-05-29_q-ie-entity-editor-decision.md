...
| **§4.4** | Identity 字段 Layer 1 reject — IdentityEditor `set/add/retract` raise `SDKStoreError`(target wording per §4.4.2 — 含 INV-7a + delete+create migration hint + ADR-IC §4.1 reference;shipped wording 跟 target 有 gap,Slice 3a follow-up update);跟 ADR-IC §4.1 INV-7c application/write_protocol 层 reject 形成 defense in depth |
