> 状态：部分实现  
> 类型：协议蓝图  
> 说明：本文档保留为历史 View 层规范；其核心语义已进入当前主线，但部分历史口径与当前实现并不完全重合。
...
硬约束（MVP，必须）：`active(A)` 的语义仅由证据层“撤销模型 `revokes/2`”决定（见 `/Users/zhenzhili/hnsm-backend/docs/断言层 证据层.md` 的 `写入协议：Field 操作 → claim/meta/active/chosen（MVP）`）；禁止以 `meta_bool(A,"active",false)` 等 meta 模型替代。
