> 状态：部分实现  
> 类型：协议蓝图  
> 说明：本文档保留为历史 Rule / Derivation 设计蓝图；其中部分语义已被当前主线吸收，但本文中的蓝图写法与当前执行契约并不完全一致。
...
accept 的唯一写入点是证据层 `claim/4`（以及对应 meta_*）。写回必须满足 `/Users/zhenzhili/hnsm-backend/docs/断言层 证据层.md` 的：
...
- 其中：
  ...
  - `E`：subject（EntityRef），必须为 `/Users/zhenzhili/hnsm-backend/docs/规范.md` 规定的 canonical `idref_v1:...` token。
...
- 其中 `R` 的生成（硬规范）：
  ...
  - `R` 必须由 `id_policy` 幂等生成：`id_policy` 定义了用于 identity 的**有序字段序列**与类型标签；其 canonicalization 与哈希规则必须复用 `/Users/zhenzhili/hnsm-backend/docs/规范.md` 的 IdentityPolicy/EntityRef 规范（不得另起一套“record uid”规则）。
...
最终硬约束：`accept` 写回必须与 `/Users/zhenzhili/hnsm-backend/docs/断言层 证据层.md` 的“撤销模型 + 幂等键 + claim_arg 默认物化策略”一致；若需要变更，必须同时 bump protocol_version 并更新 `/Users/zhenzhili/hnsm-backend/docs/导出与运行.md` 的 manifest 版本。
