> 状态：历史讨论  
> 类型：概念草案  
> 说明：本文档保留为历史审计能力讨论材料；它用于整理审计面向，不代表一套已完整落地的当前实现契约。
...
* 同时写入撤销元信息（硬规范，revokes-only）：
  ...
  - 新增撤销事件并追加 `revokes(R1, A1).`（撤销模型唯一权威见 `/Users/zhenzhili/hnsm-backend/docs/断言层 证据层.md` 的 `写入协议：Field 操作 → claim/meta/active/chosen（MVP）`；禁止 meta_bool revoked 模型）。
...
硬约束：`policy_digest` 必须等于导出包 `manifest.json` 的 `digests.policy_digest`，其计算口径以 `/Users/zhenzhili/hnsm-backend/docs/导出与运行.md` 的 `### 3.3 digest 口径（硬规范）` 为准；不允许自定义“组合 hash”。
