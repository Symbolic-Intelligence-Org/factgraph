...
| # | Lock | Source |
|---|---|---|
| **SF1** | `SchemaIndex` hosts both `identity_pred_ids` + `exists_pred_ids` frozenset fields and `protected_anchor_pred_ids` union property(ADR-IC §4.3.1 uses `_..._pred_ids` as implementation sketch;Slice 2 implementation uses **public** SchemaIndex field names per P2 #1 amend 2026-05-29 — consistent with existing public SchemaIndex fields `entities` / `field_predicates` / `predicates_by_id`)| OQ1 verdict 2026-05-29 + P2 #1 amend 2026-05-29 |
...
| **SF9** | **Two independent frozensets**(NOT single set)— `identity_pred_ids` 和 `exists_pred_ids` 概念分离(SchemaIndex public field names per P2 #1;ADR-IC §4.3.1 中 `_..._pred_ids` 是 implementation sketch);`protected_anchor_pred_ids` 仅是读 helper property;Step 2+ 移除 `:exists` 时 INV-7c 范围不收窄 | ADR-IC §4.3.1 + P2 #1 amend 2026-05-29 |
