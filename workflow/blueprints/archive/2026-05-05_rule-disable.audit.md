...
| 2026-05-06 | implemented | Post-archive hardening | Strict review found three contract gaps and all were hardened without protocol surface expansion:(P2)RuleRef-bearing `SupportArtifact.rule_refs` now rejects alongside `rule_ref_edges`; (P2)native variant evaluation `TypeError` / `ValueError` escapes are mapped to `RULE_DISABLE_NATIVE_EVAL_ERROR` at the Rule Disable runtime boundary; (P3)`RuleDisableResult.variant_rows` now uses the same strict `$`-prefixed unique binding-key validation expected of application BindingItems DTOs. The symmetric ProofFrame `rule_refs` gap is intentionally left for a separate Batch 4 hardening because Batch 5a §7 restricts ProofFrame runtime changes to the rule-action rejection guard. Verification:focused Rule Disable / Fact Overlay / ProofFrame / disabled-locator suites 145 OK;full kernel 1194 OK / 1 skipped;ruff clean;diff-check clean. |
| 2026-05-06 | implemented | Post-archive hardening round 2 | Batch 5c strict review found the same shallow RuleRef body scan in all three rule-action runtimes. Rule Disable now recursively detects `ruleref` inside compound atoms currently represented by `not` bodies,so `not(ruleref(...))` maps to `RULE_DISABLE_RULE_REF_UNSUPPORTED` instead of leaking through as `RULE_DISABLE_NATIVE_EVAL_ERROR`. Added a focused regression without changing protocol surface or RuleRef substrate. |
