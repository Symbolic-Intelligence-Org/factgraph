...
| 2026-05-06 | implemented | Post-archive hardening | Two contract drifts caught after archive and fixed without protocol DTO change:(P2)`_recheck_non_fact_step` treated any non-empty overlay as frame-relevant,downgrading still-valid frames to `unknown` when the overlay touched unrelated assertions;runtime now derives frame-relevant action indices from `pred_witnesses` once and uses that set for unrecognized future kinds. (P3)Narrative renderer emitted lines for `still_valid` verdicts,contradicting §5.5.5 Decision 2;renderer now filters them and narrative tests pin the scoped format. Verification rerun for focused ProofFrame tests,ruff,and diff check |
