...
- [x] Phase 2 lands `kernel/sdk/shells/rule_literal_replace.py`, `SDKStore.check_rule_literal_replace(...)` thin delegate, `test_sdk_rule_literal_replace.py`. (full kernel 1626 OK / 1 skipped; +17 G3 contract tests incl. extra `RuleLiteralPath` shape test; `#P1` retrofit on pre-G3 `test_no_sdk_rule_literal_replace_surface` boundary test)
- [x] Phase 3 lands `kernel/sdk/shells/rule_add_condition.py`, `SDKStore.check_rule_add_condition(...)` thin delegate, `test_sdk_rule_add_condition.py`. (full kernel 1643 OK / 1 skipped; +17 G3 contract tests incl. extra `RuleAddedAtom` shape test; `#P1` retrofit on pre-G3 `test_no_sdk_rule_add_condition_surface` boundary test; G3 8-shell scope now fully active)
- [x] Phase 4 lands `test_sdk_g3_invariants.py` (6-class mirror), updates SDK API docs (`04_api_surface.md` + `.en.md`) and application overview docs (`01_overview.md` + `_en.md`); cumulative G3 strict audit gates the close-out. (full kernel 1649 OK / 1 skipped; +6 G3 invariant tests; G1+G4+G2 snapshot branches verified untouched)
- [x] Phase 5 local close-out — §9 Outcome / Deviations filled, status `scoped → implemented`, blueprint + audit log archived under `docs/blueprints/archive/`, archive inventory updated, memory `project_g3_published.md` created. (commits `430bce2` rename + `3215083` content)
- [ ] Phase 5 publish — `v0.1-l-g3-rule-overlays-2026-05-08` (G3-only) + `v0.1-public-surface-helpers-walker-l-g1-l-g4-l-g2-l-g3-2026-05-08` (Path B combined snapshot per the post-G2 strategy) pushed to origin. **Pending explicit user authorization per `project_release_branch_invariants`** — sacred branches and publish decisions stay user-gated; auto mode does not extend to pushing to origin.
