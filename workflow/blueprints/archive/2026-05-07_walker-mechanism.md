...
- **Construction-time snapshot with lazy view creation** for mutable-source DTO walkers (e.g., `RuleSpec.where`, `CompiledDerivationPlan.body_ir`): construction freezes the source body; `IRAtomView` objects are created during traversal / lookup, not prebuilt in `IRBodyWalker.__init__`.
...
- **FrozenTupleView method semantics (Phase 2 decision):** `.first()` returns `None` on empty; `.filter(predicate=None, **attrs)` and `.find(predicate=None, **attrs)` support predicate + exact attribute matching; `require_key(value, *, key=extractor)` raises `WalkerLookupError` on miss (default extractor checks `.key` / `.pred_atom_key` / `.step_key` / `.atom_key` / `.asrt_id` / `.id`); `.filter(...)` eagerly materializes a new `FrozenTupleView`, keeping stream semantics reserved for future B3.
...
- **Equality / hash** (`#17`): surfaced DTO views use structural equality where defined; identity is not stable; `.underlying` is excluded from equality / hash; walker instances themselves use default object identity.
- **`#P1` carve-out — FrozenTupleView equality:** `FrozenTupleView` is a content-equality wrapper, not a surfaced DTO view with a separate representation. Equality and hash are defined over the wrapped tuple itself; the `.underlying` exclusion rule applies to surfaced DTO views such as `IRAtomView`, `AssertionView`, and later `ProofFrameView`.
