...
| `FR-AGENT-03` | response combinations、12-slot absence、egress guard/provider readiness under-specified | `APPLIED / FOLLOW-UP PENDING` — per-cell legal matrix、typed slot absence、pre-send structural/static-hash guard and `PF-MODEL-01` added |
...
| `FR-GOV-FU-01` | actual handoff/execution gates remained under “before scoped” | `APPLIED / FOLLOW-UP PENDING` — pre-scoped schema/readiness and post-scoped actual gates separated |
| `FR-GOV-FU-02` | handoff required future Step-2 fixture/oracle hashes | `APPLIED / FOLLOW-UP PENDING` — handoff now carries fixture-plan contract/freeze gate；actual hashes remain Step 2 outputs |
| `FR-CAP-FU-01` | review-derived operation accounting mislabeled user-accepted | `APPLIED / FOLLOW-UP PENDING` — historical `4592e491` acceptance and review-derived rows separated；all remain behind `CAP-FINAL-01` |
| `FR-EGRESS-FU-01` | dynamic second-call payload cannot match a pre-approved full digest | `APPLIED / FOLLOW-UP PENDING` — pre-authorize structural paths/classes/sizes + static hashes；record actual full-payload digest only after guard passes |
| `FR-BLIND-FU-01` | stale “12-output” acceptance wording | `APPLIED / FOLLOW-UP PENDING` — 12 expected slots distinguished from actually present candidate texts |
| `FR-REPLAY-FU-01` | implementation step implied all 13 operations target `Q02` | `APPLIED / FOLLOW-UP PENDING` — 3 distributed R0 operations separated from 10 `Q02`-representative operations |
