...
| 2026-08-01 | **终审 D1-D8 docs-only 修订完成,待 close-out** | 单一文档提交收敛:CHANGELOG 显式发布 manifest 顶层 `schema_digest` 移除、`fg.commit_changes`/`RevocationInput` 公开面;read/write guide 改用 `path=` 并把 `ledger_path` 归回 unmanaged compatibility;删除两处 manifest 校验过诺;API 补 `append_meta`;修复 quickstart 死链、namespace 名称与 layout 惰性/预留目录说明;read-after-close 裸 `RuntimeError` 登记为 known gap。无运行时代码修改;完成后停下,由 Claude 执行 §7 勾选、Outcome/Deviations、`implemented` 与归档。 |
...
3. **read-after-close 尚未 SDK 层化**:owned `FactGraph.close()` 后的写面已统一为 `SDKStoreError(code="GRAPH_CLOSED")`,但读面仍可能直通裸 `RuntimeError: Ledger is closed`。本战役终审定性为非阻塞的错误分类对称性欠账,不在 docs-only 收尾中修改代码。
