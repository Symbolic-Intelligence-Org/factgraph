...
| 2026-07-31 | application write protocol 的 `FieldValue` 明确是 JSONValue/EntityRef,不承载 `bytes`;SDK batch 对 bytes 因而落 legacy fallback | attach 上禁止 fallback 触 Ledger,改为提交前 fail-closed(零写、head 不动);非 attach 行为不变。是否扩 application DTO 支持 bytes 属 blueprint 未覆盖的协议决策,留审计/后续裁定,本阶段不自行扩面 |
