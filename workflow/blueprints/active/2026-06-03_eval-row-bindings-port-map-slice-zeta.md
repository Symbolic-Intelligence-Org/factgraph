...
- Any PyReason/core provenance timeline path found by Step 4.3 preflight that receives row-derived candidate payloads and still expects `pred_id` + `terms`.
...
- [ ] Service JSON wire preserves locked compatibility (C2 wire-envelope preservation).
- [ ] Provenance/evidence paths that need legacy candidate payloads use helper reconstruction and no longer pass `dict(row.bindings)` when the callee expects `pred_id` + `terms`.
...
- A5 — Service/provenance legacy payload compat scope — `_evaluate_row_to_dict(...)` bindings key handling plus ProbLog/PyReason/core provenance paths that expect `pred_id` + `terms`.
