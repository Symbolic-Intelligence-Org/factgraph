...
| 2026-08-02 | **内联裁定(协调方):五属性 Schema IR 形状四条(Phase 3 C0 前置)** | codex 依纪律零码停点上报。R1 形状:IR 顶层**可选** `meta_keys` 对象映射(按 meta key 字符串键控,与 schema-evolution §3.1 keyed-diff 原则一致);值 = 五属性对象;**canonical 最小形** —— 属性等于默认值即省略、空映射规范化为整段省略、`{}` 条目合法(=按默认声明);字段存在即入 identity bytes/digest。R2 默认矩阵:`reader_class=runtime / premise_eligible=false / load_policy=eager / storage_scope=claim / query_indexed=false` —— 原则:**默认=Phase 2 现状语义**,未声明 key 行为零变化,与字节不变 gate 互为语义/字节对偶。R3 初始集:`{provenance_class, origin_binding}` 为**内建隐式声明(ADR 常量)** —— 选项 (b) 自动写入违反字节 gate 即弃;选项 (c) 显式强制会把最高契约 fixture 的 premise 面打红并为零收益强迁移即弃;(a) 的漂移风险以三重钉死:模块常量 + 测试钉 + spec §5.3 注记,**内建集变更必须走 schema-evolution 决策线,代码不得单方面改**;优先级 = 显式声明覆盖内建。R4 著述入口:authoring JSON 镜像 IR `meta_keys` 节;Python 桥 = compile 入口 `meta_keys=` kwarg + 类型化 `MetaKeyPolicy`(单源校验,禁第二张表 —— 渲染/内省日后从它派生,schema-evolution §3.8 纪律);**不**扩展 per-class `class Meta`(全局关切不放实体级)。校验:属性集与值域封闭、保留/系统 key(`__system__.`/annotation 前缀/system-managed 三键/DB-owned 三键)拒绝声明、编译器负责规范化。meta_keys 参与未来 keyed diff 的接线归 schema-evolution blueprint,本 Phase 只做 compile+digest+closure。用户可否决 |
