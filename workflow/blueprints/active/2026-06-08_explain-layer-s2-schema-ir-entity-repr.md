...
| 2 | `authoring/schema_compile.py:_compile_identity_field` | identity field rows may carry description/pattern/enum metadata, but no IR repr target | copy identity `repr` so `_compile_identity_predicate` can persist it |
...
| Q-S2-C | Should relationship field `repr` persist? | Yes if S1 authoring metadata reaches relationship fields; renderer remains entity-only |
...
Entity rows store `Meta.repr`. Predicate rows store `Identity.repr` / `Field.repr` / relationship field `repr`. Identity field rows may carry `repr` as an intermediate compile payload if needed, but runtime consumers read predicate-level `repr` through `PredicateInfo`.
...
1. Step 4.2: Review signature, relationship-field persistence, and schema-digest implications.
...
5. Step 4.7: Implement on `v0.2.0-impl-schema-ir-entity-repr-2026-06-08`, forked from the S1 impl lineage.
